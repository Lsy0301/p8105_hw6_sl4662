...
``` r
train_df = sample_n(birth_data, round(4342*0.8))
test_df = anti_join(birth_data, train_df, by = "id")

ggplot(train_df, aes(x = blength, y = bwt)) + 
  geom_point() + 
  geom_point(data = test_df, color = "red", alpha = 0.5)
```

<img src="p8105_hw6_sl4662_files/figure-gfm/unnamed-chunk-6-1.png" width="90%" />
